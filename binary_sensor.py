...
from homeassistant.components.binary_sensor import ENTITY_ID_FORMAT, DEVICE_CLASSES_SCHEMA, PLATFORM_SCHEMA
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    async def async_update_data():
        ...
        """
        _LOGGER.error( "Loading resource %s", resource)
        try:
    ...
    for device, device_config in selectors.items():
        ...
        entities.append(MultiscrapeBinarySensor(
                            hass,
                            coordinator,
                            device,
                            name,
...
class MultiscrapeBinarySensor(Entity):
    ...
    def state(self):
        ...
        if self._coordinator.data is None:
            _LOGGER.error("Unable to retrieve data in state")
            return False
        return self._coordinator.data[self._key]

    @property
    def is_on(self) -> bool:
        """Return the on-state of the device."""
        if self._coordinator.data is None:
            _LOGGER.error("Unable to retrieve data in is_on")
            return False
        return self._coordinator.data[self._key]
